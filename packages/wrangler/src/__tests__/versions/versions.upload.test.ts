...
describe("versions upload", () => {
	...
	describe("--no-bundle", () => {
		...
		test("should upload without bundling", async ({ expect }) => {
			mockGetScript();
			mockUploadVersion(false);
			...
			const sourceContent = `import unresolvable from "does-not-exist"; export default { fetch() { return new Response("ok"); } }`;
			await fs.promises.writeFile("index.js", sourceContent);
	...
	describe("upload metadata", () => {
		...
		// TODO(deploy refactor): check if streaming_tail_consumers omission is intentional
		test("should exclude non-versioned fields from metadata", async ({
			expect,
		}) => {
			mockGetScript();
			const requests = mockUploadVersion(false);
			...
			writeWranglerConfig({
				name: "test-name",
				main: "./index.js",
				tail_consumers: [{ service: "regular-listener" }],
				streaming_tail_consumers: [{ service: "streaming-listener" }],
				logpush: true,
			...
			const metadata = await getMetadata(requests[requests.length - 1]);
			// tail_consumers is versioned; streaming_tail_consumers, logpush, and
			// observability are non-versioned settings handled separately by deploy
			expect(metadata.tail_consumers).toEqual([
				{ service: "regular-listener" },
			]);
			expect(metadata.streaming_tail_consumers).toBeUndefined();
			expect(metadata.logpush).toBeUndefined();
			expect(metadata.observability).toBeUndefined();
		});

		test("should set annotations from CLI flags", async ({ expect }) => {
			// --tag and --message
			mockGetScript();
			...
			writeWorkerSource();
			await runWrangler(
				'versions upload --tag v1.0.0 --message "my deploy message"'
			);
			expect(std.out).toContain("Uploaded test-name");
			...
			mockUploadVersion(true, 1, {
				...
				"workers/alias": "my-alias",
			});
			mockGetWorkerSubdomain({ enabled: true, previews_enabled: true });
			mockSubDomainRequest();
			await runWrangler("versions upload --preview-alias my-alias");
			expect(std.out).toContain("Version Preview Alias URL");
		});

		test("should include --var, --define, and --alias in upload", async ({
			expect,
		}) => {
			// --var: included as a plain_text binding in metadata
			mockGetScript();
			const varRequests = mockUploadVersion(false);
			writeWranglerConfig({ name: "test-name", main: "./index.js" });
			writeWorkerSource();
			await runWrangler("versions upload --var MY_VAR:my-value");
			const varMetadata = await getMetadata(
				varRequests[varRequests.length - 1]
			);
			expect(varMetadata.bindings).toEqual(
			...
			// --define: substitution applied at bundle time
			mockGetScript();
			mockUploadVersion(false);
			writeWranglerConfig({ name: "test-name", main: "./index.js" });
			...
			// --alias: module resolved at bundle time without bundler error
			mockGetScript();
			mockUploadVersion(false);
			writeWranglerConfig({ name: "test-name", main: "./index.js" });
			await fs.promises.writeFile(
			...
			await fs.promises.writeFile("aliased.js", `export default "aliased";`);
